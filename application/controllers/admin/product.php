...
class product extends MY_Admin_Site
{
    ...
    public function getSearchs()
    {
        if(!empty($_GET['key'])){
            $key = $_GET['key'];
            $this->load->model('admin/product_model');
            $re = $this->product_model->search($key);
            if($re == 0){
                echo"<br/>";echo"<br/>";
                echo "&nbsp;&nbsp;<a href='javascript:history.back(-1)'><b>返回</b></a>";
                echo"<br/>";echo"<br/>";
                echo '&nbsp;&nbsp;&nbsp;&nbsp;该产品编号没有注册';
            }else{
                echo"<br/>";echo"<br/>";
                echo "&nbsp;&nbsp;&nbsp;&nbsp;<a href='javascript:history.back(-1)'><b>返回</b></a>";
                echo"<br/>";echo"<br/>";
                echo "<table border='1' cellspacing='0' style='margin-left: 2%;'>";
                echo "<tr>";
                    echo "<th>产品编号</th>";
                    echo "<th>产品类别</th>";
                    echo "<th>会员名称</th>";
                ...
                echo "<tr>";
                    echo "<td>";echo "&nbsp;"; echo $re['project_num']; echo "&nbsp;"; echo "</td>";
                    echo "<td>";echo "&nbsp;"; echo $re['project_type']; echo "&nbsp;"; echo "</td>";
                    echo "<td>";echo "&nbsp;"; echo $re['username']; echo "&nbsp;"; echo "</td>";
                    echo "<td>";echo "&nbsp;"; echo $re['mobile']; echo "&nbsp;"; echo "</td>";
                    echo "<td>";echo "&nbsp;"; echo $re['add_time']; echo "&nbsp;"; echo "</td>";
                echo "</tr>";
    ...
    public function express()
    {
        $this->load->model('admin/product_model');
        if(empty($_GET['page'])){
            $result = $this->product_model->getData2(0,20);
        }else{
            $page = $_GET['page'];
            $result = $this->product_model->getData2(($page-1)*20,20);     //每页记录数
        }
        $resultCount = count($this->product_model->getData2count());
    ...
    public function express2()
    {
        ...
        $this->load->view('admin/express2.html',$datas);
    }
    /*
     * 搜索
     * 2019-11-04
    ...
    public function search()
    {
        if(!empty($_GET['keys'])){
            $keys = $_GET['keys'];
            $this->load->model('admin/product_model');
            $result = $this->product_model->searchModel($keys);
            $datas['data'] = $result;
            $this->load->view('admin/search_ex_express.html',$datas);
        }else{
            echo "<script language=javascript> alert('没有结果'); history.back();</script>";
        }
    }
    /**
     * 添加/修改快递单号
    ...
    public function editInfo()
    {
        if(!empty($_POST)){
            ...
            $data['eaddress1'] = $_POST['eaddress1'];
            $data['eaddress2'] = $_POST['eaddress2'];
   ...
   */
    ...
    public function excel($datalist)
    {
        ...
        //表头数组
        $tableheader = array('姓名','手机号','注射器编号','省','市','区/县','详细地址','提交时间','快递名称','快递单号','回访状态','状态');
        //填充表头信息
        ...
        header("Content-Disposition:attachment;filename=快递信息$date.xls");
