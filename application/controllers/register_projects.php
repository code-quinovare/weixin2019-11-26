 ...
 *
 */
class register_projects extends MY_Controller_Site
{
    public $token = '';
    public $user_id = '';
    public $open_id = '';
    ...
    {
        $this->load->model('user_model');
        //var_dump($this->user_model->findOneToken($token));
     ...
     *
     */
    public function checkUserRegnum()
    {
        $user_id = $this->user_id;
        $sql = "select count(id) num from user_project where user_id='$user_id'";
    ...
    public function doUserProject()
    {
        ...
        if(!$this->checkUserRegnum()){ echo $this->result_lib->setErrorsJson('您注册的产品数量已经达到上限');exit; }
        ...
        if($re->systemState&&($re->systemState==001)){
            ...
            if ($re) {
                ...
                if($type_code == 'P'){
                    ...
                    if(!empty($arr[1])){
                        $data['eaddress1'] = $arr[1];
                    }else{
                        $data['eaddress1'] = '';
                    }
                    if(!empty($arr[2])){
                        $data['eaddress2'] = $arr[2];
                    }else{
                        $data['eaddress2'] = '';
                    }
                    //详细地址
                    $data_add['eaddcity'] = htmlspecialchars($this->input->post('eaddcity'));
                    if($data_add['eaddcity'] != ''){
                        $data['minaddress'] = $data_add['eaddcity'];
                    } else{
                        $data['minaddress'] = '';
                    }
                ...
                echo $this->result_lib->setInfoJson('注册成功');
                exit();
            }
            echo $this->result_lib->setErrorsJson('注册失败');
            exit();
        }
        log_error($re->reply);
